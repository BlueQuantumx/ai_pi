...
def calculate_gain(nonlinearity: str, param: Optional[float] = None) -> float:
    return {
        ...
        "tanh": 5 / 3,
        "relu": math.sqrt(2.0),
        "leaky_relu": math.sqrt(2.0 / (1 + (param if param != None else 0.01) ** 2)),
    }[nonlinearity]
...
def kaiming_uniform_(
    tensor: Tensor, a: float = 0.0, mode="fan_in", nonlinearity="relu"
) -> Tensor:
    fan_in, fan_out = _calculate_fan(tensor)
    fan = {
        "fan_in": fan_in,
...
def kaiming_normal_(tensor: Tensor, a: float = 0.0, mode="fan_in", nonlinearity="relu"):
    fan_in, fan_out = _calculate_fan(tensor)
    fan = {
        "fan_in": fan_in,
        "fan_out": fan_out,
